...
namespace Awarder.BLL
{
	public class AwardsBL
	{
		...
		// init
		public AwardsBL(bool server = false)
		{
			if (server)
				awardsDAO = new AwardDAOdb();
			else
		...
		public IEnumerable<Award> GetListFromIds<T>(IEnumerable<int> ids)
		{
			return GetList().Select(a => a).Where(a => ids.Contains(a.Id));
		}
		...
		{
			awardsDAO.Update(award ?? throw new ArgumentNullException());
		}
		#endregion
